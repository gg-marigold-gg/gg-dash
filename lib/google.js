...
/** "campaign", "adset" (= ad group), or "ad". Mirrors META_LEVEL. */
const LEVEL = process.env.GOOGLE_ADS_LEVEL || "ad";
...
export async function fetchGoogle(acct, since, until) {
  ...
  // Google calls them ad groups; the dashboard calls them ad sets.
  const extraFields =
    LEVEL === "ad"
      ? "ad_group.name, ad_group.id, ad_group_ad.ad.id, ad_group_ad.ad.name, ad_group_ad.ad.final_urls,"
      : LEVEL === "adset"
  ...
  for (const chunk of chunks) {
    for (const r of chunk.results || []) {
      ...
      // Responsive ads usually have no name, so fall back to the ID.
      const adId = r.adGroupAd?.ad?.id;
      const adName = r.adGroupAd?.ad?.name || (adId ? `Ad ${adId}` : "—");

      // Google has no public page for a search ad, so the closest public URL
      // is the landing page the ad points at. Falls back to the account's ad
      // view, which needs Google Ads access.
      const finalUrls = r.adGroupAd?.ad?.finalUrls;
      const url =
        Array.isArray(finalUrls) && finalUrls.length
          ? finalUrls[0]
          : `https://ads.google.com/aw/ads?__c=${customerId}`;
