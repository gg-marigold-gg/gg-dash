...
export async function fetchGoogle(acct, since, until) {
  ...
  for (const chunk of chunks) {
    for (const r of chunk.results || []) {
      ...
      // Google Ads deep links need an internal session ID we do not have, so
      // this lands you in the right account's ad view rather than on the exact
      // ad. Better than nothing, but not the precise jump Meta gives.
      const url = `https://ads.google.com/aw/ads?__c=${customerId}`;
